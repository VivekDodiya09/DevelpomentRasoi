...
exports.cancelSubscription = async (req, res) => {
  try {
    ...
    // For Razorpay, we don't have subscription cancellation like Stripe
    // Instead, we just deactivate the tenant's subscription access
    await updateTenantSubscriptionAccess(
      user.username,
      0, // active = 0 (deactivate)
      null, // no subscription ID for Razorpay
      null, // no customer ID for Razorpay
      null, // no start date
      null // no end date
    );
...
exports.stripeWebhook = async (request, response) => {
  ...
  if (endpointSecret) {
    ...
    console.log("4 hit", request.body, signature, endpointSecret);
    try {
...
exports.createRazorpayOrder = async (req, res) => {
  try {
    ...
    // Return order details along with success redirect URL
    res.status(200).json({
      success: true,
      order,
      success_url: `${CONFIG.FRONTEND_DOMAIN}/success?session_id=${order.id}`,
      cancel_url: `${CONFIG.FRONTEND_DOMAIN}/cancelled-payment`,
    });
  } catch (error) {
...
exports.handleRazorpayWebhook = async (req, res) => {
  try {
    ...
    // Get raw body for signature verification (from express.raw middleware)
    ...
    // Handle payment events to update subscription status
    try {
      if (event.event === "payment.captured") {
        const payment = event.payload.payment.entity;
        const email = payment.email;
        const tenantId = await getTenantIdFromCustomerEmail(email);

        if (tenantId) {
          // Update subscription access to active
          await updateTenantSubscriptionAccess(
            email,
            1, // active = 1
            payment.order_id,
            null, // no stripe customer for razorpay
            new Date().toISOString().split("T")[0], // start date
            new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
              .toISOString()
              .split("T")[0] // end date (30 days from now)
          );
          ...
          console.log(
            `Subscription activated for tenant ${tenantId} via Razorpay payment`
          );
        }
      }
    } catch (dbError) {
      console.error("Error updating subscription in database:", dbError);
      // Don't return error to Razorpay, as the payment was successful
