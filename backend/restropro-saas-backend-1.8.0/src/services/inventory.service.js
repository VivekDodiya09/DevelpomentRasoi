...
exports.updateInventoryItemDB = async (
  ...
  const conn = await getMySqlPromiseConnection();
  try {
    // Get current quantity to determine status
    const [rows] = await conn.query(
      `SELECT quantity FROM inventory_items WHERE id = ? AND tenant_id = ?`,
      [itemId, tenantId]
    );
...
exports.addInventoryItemStockMovementDB = async (
  ...
  try {
    ...
    if (rows.length === 0)
      throw new Error(req.__("inventory_item_not_found_message"));
    ...
    if (newQuantity < 0)
      throw new Error(req.__("insufficient_inventory_quantity_message"));
...
exports.getCummulativeInventoryMovementsDB = async (
  type,
  from,
  to,
  tenantId
...
exports.getInventoryUsageVsCurrentStockDB = async (
  type,
  from,
  to,
  tenantId
...
exports.getVendorsDB = async (page, perPage, sort, filter, tenantId) => {
  ...
  try {
    ...
    const filterQuery = filter
      ? `WHERE (name LIKE '${filter}%' OR phone='${filter}') AND tenant_id=${tenantId}`
...
/* Purchase Orders */
exports.addItemToPurchaseOrdersDraftsDB = async (
  inventoryItemId,
  tenantId,
  quantity
) => {
...
exports.updatePurchaseOrderDraftItemQuantityDB = async (
  id,
  quantity,
  tenantId
) => {
...
exports.createPurchaseOrderDB = async (
  vendorId,
  vendorName,
  contactPerson,
  taxIdNo,
  ...
  try {
    ...
    const [orderIdRes] = await conn.query(
      `SELECT current_value FROM sequences WHERE tenant_id = ? AND table_name='inventory_purchase_orders' LIMIT 1 FOR UPDATE`,
      [tenantId]
    );
    ...
    // delete from drafts
    await conn.query(
      `DELETE FROM inventory_purchase_orders_drafts WHERE id IN (?) AND tenant_id = ?`,
      [items.map((item) => item.id), tenantId]
    );
  ...
  try {
    ...
    for (const menu of disabledMenuItems) {
      ...
      const canEnable =
        recipes.length > 0 &&
        recipes.every(
          (r) => parseFloat(r.available_quantity) >= parseFloat(r.quantity)
        );
