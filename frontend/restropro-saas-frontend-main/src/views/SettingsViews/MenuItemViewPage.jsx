...
export default function MenuItemViewPage() {
        ...
        setTimeout(() => {
          ...
          if (categoryIdRef.current) {
            categoryIdRef.current.value =
              res.data?.formattedMenuItem?.category_id;
          }
        }, 100);
  ...
  async function btnSave() {
    ...
    try {
      ...
      const message =
        error.response.data.message || t("menu_items.something_went_wrong");
      console.error(error);
      toast.dismiss();
      toast.error(message);
    }
  ...
  async function btnUpdateVariant() {
    ...
    try {
      toast.loading(t("menu_item.please_wait"));
      const res = await updateMenuItemVariant(
        id,
        variantId,
        variantTitle,
        variantPrice
  ...
  async function btnUpdateAddon() {
    ...
    try {
      toast.loading(t("menu_item.please_wait"));
      const res = await updateMenuItemAddon(
        id,
        addonId,
        addonTitle,
        addonPrice
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      toast.dismiss();
      const message =
        error?.response?.data?.message || t("menu_item.upload_image_error");
      toast.error(message);
    }
  };
  ...
  async function btnAddRecipeItem() {
    ...
    if (
      (activeAddRecipeItemTab === "variant" ||
        activeAddRecipeItemTab === "addon") &&
      !selectedRecipeData.selectedBase
    ) {
  ...
  async function btnUpdateRecipeItem() {
    ...
    if (
      (editRecipeData.baseType === "variant" ||
        editRecipeData.baseType === "addon") &&
      !editRecipeData.selectedBase
    ) {
      toast.error("Please select a variant or addon.");
      return;
    }
  ...
  const btnDeleteRecipeItem = async (
    recipeItemId,
    recipeItemVariantId,
    recipeItemAddonId
  ) => {
    ...
    try {
      toast.loading("Please wait...");
      const res = await deleteRecipeItem(
        itemId,
        recipeItemId,
        recipeItemVariantId,
        recipeItemAddonId
    ...
    <Page className="px-4 md:px-8 py-3 md:py-6">
      <div className="text-sm breadcrumbs">
        <ul>
          ...
          <li>
            <Link to="/dashboard/settings/menu-items">
              {t("menu_items.title")}
            </Link>
          </li>
          <li>{title}</li>
      ...
      <div className="my-6 flex gap-6 flex-col lg:flex-row">
        <div className="">
          <div className="relative w-32 h-32 md:w-64 md:h-64 rounded-2xl flex items-center justify-center text-2xl mb-4 text-restro-text dark:text-white bg-gradient-to-br from-restro-green/10 to-restro-bg-gray border-2 border-dashed border-restro-border-green transition-all duration-300 hover:border-restro-green hover:shadow-xl hover:scale-105 group cursor-pointer">
            {image ? (
              <div className="w-full h-full relative top-0 left-0">
                <img
                  src={imageURL}
                  alt={title}
                  className="w-full h-full absolute top-0 left-0 rounded-2xl object-cover shadow-inner"
                />
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center p-2">
              ...
              <div className="text-center opacity-60 group-hover:opacity-100 transition-opacity duration-300">
                <div className="mb-2">
                  <div className="w-12 h-12 md:w-16 md:h-16 mx-auto bg-restro-green/20 rounded-full flex items-center justify-center mb-2">
                    <IconCarrot
                      stroke={iconStroke}
                      size={24}
            ...
            <div className="absolute -top-2 -right-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <label
                htmlFor="file"
                className="bg-white dark:bg-gray-800 p-2 rounded-full shadow-lg cursor-pointer hover:bg-restro-green hover:text-white hover:shadow-xl hover:scale-110 transition-all duration-200 border-2 border-restro-green text-restro-green font-semibold text-sm hover:border-white"
                title="Upload image"
              >
                <IconUpload stroke={iconStroke} size={20} />
                <input
              ...
              {image && (
                <button
                  ...
                  className="flex items-center justify-center w-10 h-10 rounded-full shadow-lg cursor-pointer transition-all duration-200 text-white bg-red-500 hover:bg-red-600 hover:scale-110 hover:shadow-xl"
                  title="Remove image"
                >
          ...
          <div className="space-y-2">
            ...
            {net_price && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">{net_price}</span>
              </div>
            )}
            {category_id && (
              <div className="flex justify-between items-center">
          ...
          <button
            onClick={btnSave}
            className="mt-6 w-full rounded-lg transition active:scale-95 text-white hover:shadow-lg px-4 py-3 border border-restro-border-green bg-restro-green hover:bg-restro-green-button-hover font-semibold"
          >
            {t("menu_item.save")}
          </button>
        </div>
        <div className="flex-1">
          <div className="">
            <label
              htmlFor="title"
              className="text-sm mb-1 block text-restro-text"
            >
              {t("menu_item.title")}
            </label>
            <input
          ...
          {/* variants */}
          <div
            className={`collapse collapse-arrow mt-6 border dark:rounded-lg ${
              theme === "black"
                ? "border-restro-border-dark-mode"
                : "bg-gray-50 border-restro-green-light"
            }`}
          >
            <input type="checkbox" />
            ...
            <div className="collapse-content flex flex-col">
              {state.variants?.map((variant, index) => {
                return (
                  <div
                    key={variant.id}
                    className={`rounded-2xl flex items-center justify-between transition p-2 cursor-pointer ${
                      theme === "black"
                        ? "hover:bg-restro-card-iconbg border-restro-border-dark-mode "
                        : "hover:bg-gray-100"
                    }`}
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        {t("menu_item.price")}: {variant.price}
                      </p>
                    </div>
                    <div className="flex items-center">
                      <button
                        onClick={() => {
                          btnShowVariantUpdate(
                            variant.id,
                            variant.title,
                            variant.price
                          );
              ...
              <button
                onClick={() =>
                  document.getElementById("modal-add-variant").showModal()
                }
                className="btn btn-sm mt-4 rounded-xl bg-restro-bg-gray hover:bg-restro-button-hover"
              >
                {t("menu_item.add_variant")}
              </button>
            </div>
          </div>
          {/* variants */}
          ...
          >
            ...
            <div className="collapse-content flex flex-col">
                ...
                return (
                  ...
                  >
                    <div className="flex-1">
                      <p>{addon.title}</p>
                      <p className="text-xs text-gray-500">
                        {t("menu_item.price_increase")}: +{addon.price}
                      </p>
                    </div>
                    <div className="flex items-center">
                      <button
              ...
              <button
                onClick={() =>
                  document.getElementById("modal-add-addon").showModal()
                }
                className="btn btn-sm mt-4 rounded-xl bg-restro-bg-gray hover:bg-restro-button-hover"
              >
                {t("menu_item.add_addon")}
              </button>
            </div>
          </div>
          {/* addons */}
          ...
          >
            <input type="checkbox" />
            <div className="collapse-title font-medium">Recipe Items</div>
            <div className="collapse-content flex flex-col">
                  ...
                  >
                    <div className="flex-1">
                      <p className="flex items-center">
                        ...
                        {item.variant_title && (
                          <span
                            className={`ml-2 px-2 text-xs rounded-full bg-gray-200`}
                          >
                            {item.variant_title}
                          </span>
                        )}
                        {item.addon_title && (
                          <span
                            className={`ml-2 px-2 text-xs rounded-full ${
                              theme === "black"
                                ? "bg-restro-gray-dark-mode text-gray-400"
                                : "bg-gray-200"
                            }`}
                          >
                            {item.addon_title}
                    ...
                    <div className="flex items-center">
                      <button
                        onClick={() => {
                          ...
                          setTimeout(() => {
                            if (quantityEditRef.current)
                              quantityEditRef.current.value = item.quantity;
                          }, 100);
                          const activeTab = item.variant_id
                            ? "variant"
                            : item.addon_id
                            ? "addon"
                            : "item";
                          ...
                          document
                            .getElementById("modal-edit-recipe-item")
                            .showModal();
                        }}
              ...
              <button
                onClick={() =>
                  document.getElementById("modal-add-recipe-item").showModal()
                }
                className="btn btn-sm mt-4 rounded-xl bg-restro-bg-gray hover:bg-restro-button-hover"
              >
                Add Item
        ...
        <div className="modal-box border border-restro-border-green dark:rounded-2xl">
          ...
          <div className="mt-4">
            ...
            <input
              ...
              name="title"
              className="text-sm w-full rounded-lg px-4 py-2 border border-restro-border-green dark:bg-black focus:outline-restro-border-green bg-restro-gray"
              placeholder={t("menu_item.variant_title")}
            />
          </div>

          <div className="my-4">
            <label htmlFor="price" className="mb-1 block text-gray-500 text-sm">
              {t("menu_item.variant_price")}
            </label>
            <input
              ref={variantPriceRef}
              type="number"
              name="price"
              className="text-sm w-full rounded-lg px-4 py-2 border border-restro-border-green dark:bg-black focus:outline-restro-border-green bg-restro-gray"
              placeholder={t("menu_item.variant_price")}
            />
            <p className="text-xs text-gray-500 mt-1">
              {t("menu_item.final_price_note")}
            </p>
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              ...
              <button
                ...
                className="rounded-xl transition active:scale-95 hover:shadow-lg px-4 py-3 ml-3 text-white border border-restro-border-green bg-restro-green hover:bg-restro-green-button-hover"
              >
                {t("menu_item.save")}
              </button>
            </form>
        ...
        <div className="modal-box border border-restro-border-green dark:rounded-2xl">
          ...
          <div className="mt-4">
            ...
            <input
              ...
              name="title"
              className="text-sm w-full rounded-lg px-4 py-2 border border-restro-border-green dark:bg-black focus:outline-restro-border-green bg-restro-gray"
              placeholder={t("menu_item.addon_title")}
            />
          </div>

          <div className="my-4">
            <label htmlFor="price" className="mb-1 block text-gray-500 text-sm">
              {t("menu_item.addon_price")}
            </label>
            <input
              ref={addonPriceRef}
              type="number"
              name="price"
              className="text-sm w-full rounded-lg px-4 py-2 border border-restro-border-green dark:bg-black focus:outline-restro-border-green bg-restro-gray"
              placeholder={t("menu_item.addon_price")}
            />
        ...
        <div className="modal-box border border-restro-border-green dark:rounded-2xl">
          ...
          {activeAddRecipeItemTab !== "item" && (
            <p className="text-xs text-gray-500 mb-4">
              <strong>Note:</strong> Add only those extra inventory items that
              are <em>not included</em> in the base item recipe.
            </p>
          )}
